...
Press the center of the side rocker to generate the next deterministic variant
for the current day or active hour. Between updates the ESP32 uses light sleep,
waking from the rocker, touchscreen, USB serial, or the next scheduled interval.

Press the rocker left or right to cycle the current recipe preference: `ALL`,
`CELLULAR`, `PIXEL FIELD`, `SUBDIVISION`, `DITHER`, and `MURMURATION`.
...
Captures are saved as `slow-draw-001.png`, `slow-draw-002.png`, and so on. Each
...
rendered incorrectly. The raw capture is preserved; repeated previews are saved
as `slow-draw-001-eink.png`, `slow-draw-001-eink-002.png`, and so on.
