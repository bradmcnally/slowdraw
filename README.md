...
- Cellular Aggregate grows connected cells from a random walk; grayscale
...
updates, the ESP32 and e-paper controller use light sleep.
...
When the device is awake, tap the artwork to open `OPTIONS`. It shows the
...
shown in `OPTIONS`, without changing the RTC or saved current print:
